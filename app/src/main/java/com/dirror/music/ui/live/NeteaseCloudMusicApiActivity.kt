...
class NeteaseCloudMusicApiActivity : BaseActivity() {
    ...
    override fun initView() {
        with(binding) {
            switcherEnableService.setChecked(App.mmkv.decodeBool(Config.USER_NETEASE_CLOUD_MUSIC_API_ENABLE, true))
            etService.setText(App.mmkv.decodeString(Config.USER_NETEASE_CLOUD_MUSIC_API_URL, API_AUTU))
        }
    }
...
//        App.mmkv.encode(Config.USER_NETEASE_CLOUD_MUSIC_API_URL, binding.etService.text.toString())
        App.mmkv.encode(Config.USER_NETEASE_CLOUD_MUSIC_API_URL, API_AUTU)
